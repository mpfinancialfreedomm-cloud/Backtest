        ...
        if uploaded is not None:
            ...
            if st.button('Run Backtest'):
                with st.spinner('Running backtest — this may take some time...'):
                    ...
                    # Plot per-trade profit and cumulative profit (robust handling)
                    try:
                        profit_series = pd.to_numeric(out['profit'], errors='coerce').fillna(0)
                        # use signal_date for x-axis if present and parseable
                        if 'signal_date' in out:
                            idx = pd.to_datetime(out['signal_date'], errors='coerce')
                            if not idx.isna().all():
                                profit_series.index = idx
                        profit_series.name = 'profit'
